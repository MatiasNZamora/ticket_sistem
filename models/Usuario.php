<?php
    ...
    class Usuario extends Conectar {
        ...
        public function login(){
            ...
            if(isset($_POST["enviar"])){
                ...
                if(empty($correo) and empty($pass)){
                    ...
                    $resultado = $stmt-> fetch(); // almacenamos el resultado.
                    
                    if( is_array($resultado) and count($resultado)>0 ){ // verificamo si es un array y si es mayor a 0.
        ...
        public function delete_usuario($usu_id){
            $conectar = parent::Conexion();
            parent::set_names();
            $sql="UPDATE tm_usuario SET estado='0', fech_elim=now() where usu_id=?";
            $sql=$conectar->prepare($sql);
        ...
        public function get_usuario_grafico($usu_id){
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT tm_categoria.cat_nom as nom,COUNT(*) AS total FROM tm_ticket JOIN tm_categoria 
                ON tm_ticket.cat_id = tm_categoria.cat_id WHERE tm_ticket.estado = 1 and tm_ticket.usu_id = ?  
                GROUP BY tm_categoria.cat_nom ORDER BY total DESC";
